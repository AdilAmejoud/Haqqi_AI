...
def index_from_json(json_file: str):
    ...
    for i, item in enumerate(laws):
        ...
        # Format personnalisé : {"law_name": ..., "text": ..., "domain": ..., "article": ...}
            ...
            raw_text = item["text"]
            # Supporte text comme string ou liste (de strings ou d'entiers)
            text     = " ".join(str(t) for t in raw_text) if isinstance(raw_text, list) else str(raw_text)
            text     = text.strip()
            if not text or text in ("0", "1", "None"):
