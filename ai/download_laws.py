...
def download_mizanqa():
    ...
    ds = load_dataset("adlbh/MizanQA-v0")
    ...
    for split in ds:
        for item in ds[split]:
            ...
            # options is stored as a string repr of a list
            try:
                options = ast.literal_eval(item.get("options", "[]"))
            except Exception:
                options = []

            # answer is stored as a string repr of a list of indices e.g. '[2]'
            try:
                answer_indices = ast.literal_eval(item.get("answer", "[]"))
                correct_answers = [options[i] for i in answer_indices if isinstance(i, int) and i < len(options)]
            except Exception:
                correct_answers = []

            correct_text = " | ".join(correct_answers) if correct_answers else ""
            all_options_text = " | ".join(options) if options else ""
            text = f"سؤال: {question}\nالإجابة الصحيحة: {correct_text}\nالخيارات: {all_options_text}"
